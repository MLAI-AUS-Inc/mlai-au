...
export default function UpdateCoverEditor({ backendBaseUrl, companyId, scopeKey, updateText, value, onChange }: Props) {
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white" aria-labelledby={`${id}-title`}>
      <div className="flex flex-wrap items-start justify-between gap-3 px-5 pt-6 sm:px-7">
        <div>
          <p className="mb-2 text-[11px] font-bold uppercase tracking-[0.18em] text-teal-700">A little personality</p>
          <h2 id={`${id}-title`} className="text-xl font-bold tracking-tight text-slate-950 sm:text-2xl">Give your update a cover</h2>
          <p className="mt-2 max-w-xl text-sm leading-6 text-slate-600">A moment from your month, or something a little more imaginative.</p>
        </div>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-500">Optional</span>
      </div>

      <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.15fr_1fr] lg:items-center">
        <div
        ...
        >
          {displayed ? <img src={displayed.url} alt={displayed.alt || "Update cover preview"} className="h-full w-full object-cover" /> : (
            <div className="absolute inset-0 overflow-hidden bg-[radial-gradient(ellipse_at_20%_10%,#f6dfba,transparent_60%),linear-gradient(140deg,#e4efe6,#f2e6d6)]" aria-hidden="true">
              ...
              <span className="absolute bottom-4 left-4 rounded-full bg-white/85 px-3 py-1.5 text-[11px] font-medium text-slate-700 backdrop-blur">Your story, in an image</span>
            </div>
          )}
          {busy ? <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-white/80 px-6 text-center backdrop-blur-sm" role="status">
            ...
            <p className="text-sm font-semibold text-slate-900">{state === "generating" ? "Finding the shape of your story…" : "Adding your cover…"}</p>
            <p className="text-xs leading-5 text-slate-600">{state === "generating" ? "This can take a few minutes. You can keep writing." : "Preparing an image that looks good at every size."}</p>
          </div> : null}
        ...
        <div className="min-w-0">
          ...
          {candidate ? (
            <>
              ...
              <button type="button" disabled={busy} onClick={() => { onChange(candidate); setCandidate(null); remember(null); }} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-teal-700 px-5 py-3 text-sm font-semibold text-white transition hover:bg-teal-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-700"><CheckIcon className="h-4 w-4" />Use this image</button>
            ...
            <>
              ...
              <p className="mt-2 text-center text-xs text-slate-500">Or drop it onto the preview · JPG, PNG, WebP · up to 10 MB</p>
              <div className="my-4 flex items-center gap-3 text-[11px] uppercase tracking-widest text-slate-400"><span className="h-px flex-1 bg-slate-200" />or imagine something<span className="h-px flex-1 bg-slate-200" /></div>
            </>
          )}
          <button type="button" disabled={busy || !canGenerate} onClick={() => void generate()} aria-describedby={`${id}-generation-help`} className={`${candidate ? "mt-4" : ""} flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#143e3b] px-4 py-3 text-sm font-semibold text-white transition hover:bg-teal-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-700 disabled:opacity-50`}>
          ...
          <p id={`${id}-generation-help`} className="mt-2 text-xs leading-5 text-slate-500">{canGenerate ? "Inspired by your update. Beautiful, a little abstract, and yours to choose. Made with GPT Image 2.5." : "Add a few lines to your update first. We’ll use your story to inspire the image."}</p>
      ...
      {error ? <p role="alert" className="mx-5 mb-5 rounded-xl bg-rose-50 px-4 py-3 text-sm text-rose-800 sm:mx-7">{error}</p> : null}
      {!value && !candidate ? <p className="border-t border-slate-100 px-5 py-3 text-xs leading-5 text-slate-500 sm:px-7"><PhotoIcon className="mr-1.5 inline h-3.5 w-3.5" />No image? Your update will use a simple date cover.</p> : null}
    </section>
  );
}
