...
export default function AuthorBio({ author, authors, className = '' }: AuthorBioProps) {
    ...
    return (
        <div className={`not-prose mt-12 rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white p-8 ${className}`}>
            ...
            <div className={`grid gap-6 ${authorList.length > 1 ? 'md:grid-cols-3' : 'md:grid-cols-1 md:max-w-2xl'}`}>
                {authorList.map((person, idx) => (
                    <div
                        key={person.name + idx}
                        className="flex flex-col items-center text-center p-4 rounded-xl bg-white shadow-sm border border-gray-100 hover:shadow-md transition-shadow"
                    >
                        {person.avatarUrl && (
                            <img
                        ...
                        {person.role && (
                            <p className="text-sm text-[--brand-ink] font-medium mb-2">{person.role}</p>
                        )}
                        {person.bio && (
