...
export async function action({ request, context }: ActionFunctionArgs) {
    ...
    try {
        ...
        return Response.json({ error: `Invalid JSON: ${String(error)}` }, { status: 400 });
    }
    ...
    if (filtered.length === 0) {
        return Response.json({ error: "Body must include `urls` as a non-empty string array." }, { status: 400 });
    }
    if (invalidUrls.length > 0) {
        return Response.json({ error: `Invalid URLs: ${invalidUrls.join(", ")}` }, { status: 400 });
    }
