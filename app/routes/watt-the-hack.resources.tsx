...
function AdvancedTrackDocs() {
  return (
    <Section id="advanced-track" title="Advanced Track Technical Docs" eyebrow="For Python Coders">
      <div className="space-y-8">
        
        <Card>
          <h3 className="text-2xl font-bold text-white mb-4">Grid Components</h3>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-[#7cffc3]/20 bg-[#0d211d] p-4">
              <h4 className="font-semibold text-[#7cffc3] mb-2">City Demand</h4>
              <p className="text-sm text-white/70">The baseline electricity consumption of the city (in MW). Your primary directive is to meet this demand at all times.</p>
            </div>
            <div className="rounded-xl border border-[#7cffc3]/20 bg-[#0d211d] p-4">
              <h4 className="font-semibold text-[#7cffc3] mb-2">Solar Generation</h4>
              <p className="text-sm text-white/70">Renewable energy generated by the sun (in MW). You can choose to <strong>curtail</strong> (disconnect) the solar if you're hitting the grid's export caps.</p>
            </div>
            <div className="rounded-xl border border-[#7cffc3]/20 bg-[#0d211d] p-4">
              <h4 className="font-semibold text-[#7cffc3] mb-2">Battery & Inverter</h4>
              <p className="text-sm text-white/70">Stores excess energy. Bound by two constraints: its <strong>Capacity</strong> and its <strong>Inverter Limit</strong> (max charge/discharge rate).</p>
            </div>
            <div className="rounded-xl border border-[#7cffc3]/20 bg-[#0d211d] p-4">
              <h4 className="font-semibold text-[#7cffc3] mb-2">Diesel Generator</h4>
              <p className="text-sm text-white/70">An expensive emergency backup generator. Dispatch it to prevent blackouts when solar, battery, and grid imports are insufficient.</p>
            </div>
            <div className="rounded-xl border border-[#7cffc3]/20 bg-[#0d211d] p-4">
              <h4 className="font-semibold text-[#7cffc3] mb-2">External Grid</h4>
              <p className="text-sm text-white/70">The city's connection to the broader energy market. It has strict Import Caps and Export Caps.</p>
            </div>
            <div className="rounded-xl border border-[#7cffc3]/20 bg-[#0d211d] p-4">
              <h4 className="font-semibold text-[#7cffc3] mb-2">FCAS Market</h4>
              <p className="text-sm text-white/70">Frequency Control Ancillary Services. Reserve a portion of your battery's <strong>inverter capacity</strong> to provide stability, earning steady revenue.</p>
            </div>
          </div>
        </Card>

        <Card>
          <h3 className="text-2xl font-bold text-white mb-4">Financials & Constraints</h3>
          <div className="space-y-4">
            <div className="border-l-2 border-[#7cffc3] pl-4">
              <h4 className="font-semibold text-white">Dynamic Import Tariff</h4>
              <p className="text-sm text-white/70">Cost to import power from the external grid. Fluctuates wildly based on the time of day and market conditions. You can read it via <code>state["price"]</code>.</p>
            </div>
            <div className="border-l-2 border-[#7cffc3] pl-4">
              <h4 className="font-semibold text-white">Static Export Tariff</h4>
              <p className="text-sm text-white/70">When you export excess solar or battery power to the grid, you earn this flat rate.</p>
            </div>
            <div className="border-l-2 border-[#ffd166] pl-4">
              <h4 className="font-semibold text-white">Throughput Budget</h4>
              <p className="text-sm text-white/70">Batteries degrade. In some scenarios, you are given a total throughput budget (MWh). Every MWh you charge or discharge permanently consumes this budget.</p>
            </div>
            <div className="border-l-2 border-red-400 pl-4">
              <h4 className="font-semibold text-white">Blackout & Overvoltage Penalties</h4>
              <p className="text-sm text-white/70">If Demand &gt; (Solar + Battery + Diesel + Import), a blackout occurs. If (Solar + Battery) &gt; (Demand + Export), overvoltage occurs. Both incur massive penalties.</p>
            </div>
          </div>
        </Card>

        <Card>
          <h3 className="text-2xl font-bold text-white mb-4">Writing a Controller</h3>
          <p className="leading-relaxed text-white/70 mb-4">
            Your controller runs every timestep (15 minutes). You can write a simple function or a class-based agent.
          </p>
          <div className="space-y-6">
            <div>
              <h4 className="font-semibold text-[#7cffc3] mb-2">1. The Entrypoint</h4>
              <p className="text-sm text-white/70">
                You can submit a standard Python function named <code>controller(state)</code>, or a class that contains a <code>step(self, state)</code> method. Our evaluator automatically detects the entrypoint from your file.
              </p>
            </div>
            <div>
              <h4 className="font-semibold text-[#7cffc3] mb-2">2. Persistent Memory</h4>
              <p className="text-sm text-white/70">
                If you need to remember past prices or actions:
                <br/>• <strong>Function-based:</strong> Use module-level variables defined outside your function.
                <br/>• <strong>Class-based:</strong> Initialize <code>self.memory = []</code> in your <code>__init__</code> and access it inside <code>step(self, state)</code>.
              </p>
            </div>
            <div>
              <h4 className="font-semibold text-[#7cffc3] mb-2">3. Current State vs Forecast</h4>
              <p className="text-sm text-white/70">
                The core fields (<code>demand</code>, <code>solar</code>, <code>price</code>, <code>soc</code>) contain <strong>ONLY the current timestep's values</strong>. You cannot magically see the future here. If you want to look ahead, you must use the <code>state["forecast"]</code> array (usually 24 hours ahead) - but beware, forecasts contain realistic noise and cyberattack spoofing!
              </p>
            </div>
            <div>
              <h4 className="font-semibold text-[#7cffc3] mb-2">4. The Action Dictionary</h4>
              <p className="text-sm text-white/70">
                Return a dictionary dictating your grid commands:
                <br/>• <code>battery_flow_mw</code>: Positive = discharge to grid, Negative = charge from grid.
                <br/>• <code>emergency_generator</code>: MW of diesel to dispatch.
                <br/>• <code>curtail_solar</code>: MW of solar to intentionally disconnect.
                <br/>• <code>fcas_reserve_mw</code>: MW of inverter capacity to lock for frequency control.
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <h3 className="text-2xl font-bold text-white mb-4">Scoring & Leaderboard</h3>
          <p className="leading-relaxed text-white/70 mb-4">
            Your objective is to minimize total cost. 
          </p>
          <div className="rounded-xl border border-[#ffd166]/40 bg-[#ffd166]/10 p-4">
            <h4 className="font-semibold text-[#ffd166] mb-2">Important Note on Leaderboard Scoring</h4>
            <p className="text-sm text-white/80">
              The leaderboard score is normalized against a baseline. <strong>To receive a score greater than 0 points, your controller must perform strictly better than the provided reference template for the scenario.</strong> Submitting the default template will result in 0 points!
            </p>
          </div>
        </Card>

        <Card>
          <h3 className="text-2xl font-bold text-white mb-4">Playtesting & Submissions</h3>
          <p className="leading-relaxed text-white/70 mb-4">
            <strong>Local Engine:</strong> You can install the headless engine locally to test your controller before submitting:
          </p>
          <pre className="overflow-x-auto rounded-lg bg-[#061412] p-4 font-mono text-sm text-slate-300 mb-4">
            <code>pip install git+https://github.com/AaronEliasZachariah/Watt-The-Hack-Engine-Public.git</code>
            <br/>
