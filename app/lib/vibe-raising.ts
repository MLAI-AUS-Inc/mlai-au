...
const ADMIN_OVERVIEW_PATH = "/api/v1/vibe-raising/admin/overview/";
const ADMIN_UPDATES_PATH = "/api/v1/vibe-raising/admin/updates/";
...
const ADMIN_STAT_DEFS = [
  { key: "startupsCreatingUpdates", snakeKey: "startups_creating_updates", label: "Startups creating updates" },
  { key: "updatesCreated", snakeKey: "updates_created", label: "Updates created" },
  { key: "publishedUpdates", snakeKey: "published_updates", label: "Published updates" },
  { key: "draftsInProgress", snakeKey: "drafts_in_progress", label: "Drafts in progress" },
  { key: "investorsReached", snakeKey: "investors_reached", label: "Investors reached" },
];
...
function normalizeAdminTrendDirection(value: unknown): VibeRaisingAdminStatCard["trendDirection"] {
  const normalized = String(value || "").trim().toLowerCase();
  if (["up", "increase", "increased", "positive"].includes(normalized)) return "up";
  if (["down", "decrease", "decreased", "negative"].includes(normalized)) return "down";
  if (["neutral", "flat", "same"].includes(normalized)) return "neutral";
  return normalized || null;
}

function normalizeAdminStatCard(raw: unknown, key: string, label: string): VibeRaisingAdminStatCard {
  const payload = asRecord(raw);
  const rawValue = payload
    ? payload.value ?? payload.count ?? payload.total
    : raw;
  const numericValue = asNullableNumber(rawValue);
  const value =
    asNullableString(payload?.valueText) ??
    asNullableString(payload?.value_text) ??
    asNullableString(rawValue) ??
    (numericValue === null ? "0" : formatAdminNumber(numericValue));

  return {
    key: asNullableString(payload?.key) ?? key,
    label: asNullableString(payload?.label) ?? label,
    value,
    trendLabel:
      asNullableString(payload?.trendLabel) ??
      asNullableString(payload?.trend_label) ??
      asNullableString(payload?.deltaLabel) ??
      asNullableString(payload?.delta_label),
    trendDirection: normalizeAdminTrendDirection(
      payload?.trendDirection ?? payload?.trend_direction ?? payload?.direction,
    ),
  };
}

function normalizeAdminStats(raw: unknown): VibeRaisingAdminStatCard[] {
  if (Array.isArray(raw)) {
    return raw.map((item, index) => {
      const fallback = ADMIN_STAT_DEFS[index] ?? { key: `stat-${index}`, label: "Metric" };
      return normalizeAdminStatCard(item, fallback.key, fallback.label);
    });
  }

  const payload = asRecord(raw) ?? {};
  return ADMIN_STAT_DEFS.map((definition) =>
    normalizeAdminStatCard(
      payload[definition.key] ?? payload[definition.snakeKey],
      definition.key,
      definition.label,
    ),
  );
...
function normalizeAdminTimePoints(raw: unknown): VibeRaisingAdminTimePoint[] {
  const source = asRecord(raw)?.points ?? raw;
  if (!Array.isArray(source)) return [];

  return source
    .map((item): VibeRaisingAdminTimePoint | null => {
      const payload = asRecord(item);
      if (!payload) return null;
      const value = asNullableNumber(payload.value ?? payload.count ?? payload.total);
      if (value === null) return null;
      const label =
        asNullableString(payload.label) ??
        asNullableString(payload.month) ??
        asNullableString(payload.period) ??
        asNullableString(payload.date) ??
        "";
    ...
    .filter((point): point is VibeRaisingAdminTimePoint => Boolean(point));
}

function normalizeAdminBreakdownItems(raw: unknown): VibeRaisingAdminBreakdownItem[] {
  const source = asRecord(raw)?.items ?? raw;
  if (Array.isArray(source)) {
    return source
      .map((item): VibeRaisingAdminBreakdownItem | null => {
        const payload = asRecord(item);
        if (!payload) return null;
        const label = asNullableString(payload.label) ?? asNullableString(payload.name);
        const value = asNullableNumber(payload.value ?? payload.count ?? payload.total);
        if (!label || value === null) return null;
        const percentage = asNullableNumber(payload.percentage ?? payload.percent);
        return {
          label,
          value,
          valueText:
          ...
          percentage,
          percentageText:
            asNullableString(payload.percentageText) ??
            asNullableString(payload.percentage_text) ??
            (percentage === null ? null : `${percentage}%`),
        };
      })
      .filter((item): item is VibeRaisingAdminBreakdownItem => Boolean(item));
  }

  const payload = asRecord(source);
  if (!payload) return [];
  return Object.entries(payload)
    .map(([label, value]): VibeRaisingAdminBreakdownItem | null => {
...
function normalizeAdminActionLabel(status: string): string {
  const normalized = status.toLowerCase().replace(/[\s-]+/g, "_");
  if (normalized === "published") return "View";
  if (["draft", "private"].includes(normalized)) return "Open Draft";
  return "Review";
}

function normalizeAdminUpdateSummary(raw: unknown): VibeRaisingAdminUpdateSummary | null {
  const payload = asRecord(unwrapPayload(raw));
  if (!payload) return null;
  const company = asRecord(payload.company) ?? asRecord(payload.startup);
  const update = asRecord(payload.update) ?? asRecord(payload.monthlyUpdate) ?? asRecord(payload.monthly_update);
  const founder = asRecord(payload.founder) ?? asRecord(payload.user);
  const id =
    asNullableString(payload.id) ??
    asNullableString(payload.updateId) ??
    asNullableString(payload.update_id) ??
    asNullableString(update?.id);
  if (!id) return null;
  const status =
    asNullableString(payload.status) ??
    asNullableString(update?.status) ??
    asNullableString(update?.visibility) ??
    "draft";

  return {
    id,
    startupName:
      asNullableString(payload.startupName) ??
      asNullableString(payload.startup_name) ??
      asNullableString(payload.companyName) ??
      asNullableString(payload.company_name) ??
      asNullableString(company?.name) ??
      "Unknown startup",
    startupAvatarUrl:
      asNullableString(payload.startupAvatarUrl) ??
      asNullableString(payload.startup_avatar_url) ??
      asNullableString(payload.companyAvatarUrl) ??
      asNullableString(payload.company_avatar_url) ??
      asNullableString(company?.avatarUrl) ??
      asNullableString(company?.avatar_url),
    updateMonth:
      asNullableString(payload.updateMonth) ??
      asNullableString(payload.update_month) ??
      asNullableString(update?.month) ??
      "Update",
    status,
    lastUpdatedAt:
      asNullableString(payload.lastUpdatedAt) ??
      asNullableString(payload.last_updated_at) ??
      asNullableString(payload.updatedAt) ??
      asNullableString(payload.updated_at) ??
      asNullableString(update?.updatedAt) ??
      asNullableString(update?.updated_at) ??
      asNullableString(update?.date),
    founderName:
      asNullableString(payload.founderName) ??
      asNullableString(payload.founder_name) ??
      asNullableString(founder?.fullName) ??
      asNullableString(founder?.full_name) ??
      asNullableString(founder?.name),
    actionLabel:
      asNullableString(payload.actionLabel) ??
      asNullableString(payload.action_label) ??
      normalizeAdminActionLabel(status),
    companyId:
      asNullableString(payload.companyId) ??
      asNullableString(payload.company_id) ??
      asNullableString(company?.id),
  };
}

function normalizeAdminUpdateSummaries(raw: unknown): VibeRaisingAdminUpdateSummary[] {
  if (!Array.isArray(raw)) return [];
  return raw
    .map(normalizeAdminUpdateSummary)
    .filter((update): update is VibeRaisingAdminUpdateSummary => Boolean(update));
}

function normalizeVibeRaisingAdminOverview(raw: unknown): VibeRaisingAdminOverview {
  const payload = asRecord(unwrapPayload(raw)) ?? {};
  const stats = normalizeAdminStats(payload.stats ?? payload.cards ?? payload.metricCards ?? payload.metric_cards);
  const recentUpdates = normalizeAdminUpdateSummaries(
    payload.recentUpdates ?? payload.recent_updates ?? payload.updates,
  );
  const reviewCount =
    asNullableNumber(payload.reviewCount ?? payload.review_count ?? asRecord(payload.review)?.count) ??
    recentUpdates.filter((update) => ["ready", "review", "in_review"].includes(update.status.toLowerCase())).length;

  return {
    stats,
    updatesOverTime: normalizeAdminTimePoints(
      payload.updatesOverTime ?? payload.updates_over_time ?? payload.timeline,
    ),
    updatesByStage: normalizeAdminBreakdownItems(
      payload.updatesByStage ?? payload.updates_by_stage ?? payload.byStage ?? payload.by_stage,
    ),
    updatesByIndustry: normalizeAdminBreakdownItems(
      payload.updatesByIndustry ?? payload.updates_by_industry ?? payload.byIndustry ?? payload.by_industry,
    ),
    recentUpdates,
    reviewCount,
  };
...
function normalizeVibeRaisingAdminUpdatesList(raw: unknown): VibeRaisingAdminUpdatesListResponse {
  const payload = asRecord(unwrapPayload(raw)) ?? {};
  const updates = normalizeAdminUpdateSummaries(
    payload.updates ?? payload.results ?? payload.recentUpdates ?? payload.recent_updates,
  );
  const total = asNullableNumber(payload.total ?? payload.count) ?? updates.length;
  const page = asNullableNumber(payload.page) ?? 1;
  const pageSize = asNullableNumber(payload.pageSize ?? payload.page_size ?? payload.perPage ?? payload.per_page) ?? updates.length;
  ...
  return {
    ...
    page,
    pageSize,
    hasNext: Boolean(payload.hasNext ?? payload.has_next ?? payload.next),
    hasPrevious: Boolean(payload.hasPrevious ?? payload.has_previous ?? payload.previous),
  };
}

function normalizeVibeRaisingAdminUpdateDetail(raw: unknown): VibeRaisingAdminUpdateDetail {
  const payload = asRecord(unwrapPayload(raw)) ?? {};
  const rawSummary = payload.summary ?? payload.updateSummary ?? payload.update_summary ?? payload;
  const summary = normalizeAdminUpdateSummary(rawSummary) ?? {
    id: asNullableString(payload.id) ?? "update",
    startupName: "Unknown startup",
    updateMonth: "Update",
    status: "draft",
    actionLabel: "View",
  };
  const rawUpdate = payload.update ?? payload.monthlyUpdate ?? payload.monthly_update ?? payload;
  const company = payload.company ? normalizeCompany(payload.company) : null;
  const founderPayload = asRecord(payload.founder) ?? asRecord(payload.user);

  return {
    summary,
    update: normalizeMonthlyUpdate(rawUpdate),
    company,
    founder: founderPayload
      ? {
          name:
            asNullableString(founderPayload.name) ??
            asNullableString(founderPayload.fullName) ??
            asNullableString(founderPayload.full_name),
          email: asNullableString(founderPayload.email),
        }
      : null,
  };
}

const DEV_ADMIN_OVERVIEW_STUB: VibeRaisingAdminOverview = {
  stats: [
    { key: "startupsCreatingUpdates", label: "Startups creating updates", value: "248", trendLabel: "14% vs last month", trendDirection: "up" },
    { key: "updatesCreated", label: "Updates created", value: "412", trendLabel: "18% vs last month", trendDirection: "up" },
    { key: "publishedUpdates", label: "Published updates", value: "276", trendLabel: "13% vs last month", trendDirection: "up" },
    { key: "draftsInProgress", label: "Drafts in progress", value: "136", trendLabel: "6% vs last month", trendDirection: "down" },
    { key: "investorsReached", label: "Investors reached", value: "9.8K", trendLabel: "21% vs last month", trendDirection: "up" },
  ],
  updatesOverTime: [
    { label: "Jan 2026", value: 260 },
    { label: "Feb 2026", value: 302 },
    { label: "Mar 2026", value: 284 },
    { label: "Apr 2026", value: 342 },
    { label: "May 2026", value: 379 },
    { label: "Jun 2026", value: 412 },
  ],
  updatesByStage: [
    { label: "Pre-seed", value: 112, percentage: 27, percentageText: "27%" },
    { label: "Seed", value: 98, percentage: 24, percentageText: "24%" },
    { label: "Series A", value: 76, percentage: 18, percentageText: "18%" },
    { label: "Series B+", value: 64, percentage: 16, percentageText: "16%" },
  ],
  updatesByIndustry: [
    { label: "SaaS", value: 114, percentage: 28, percentageText: "28%" },
    { label: "AI/ML", value: 92, percentage: 22, percentageText: "22%" },
    { label: "Healthtech", value: 64, percentage: 16, percentageText: "16%" },
    { label: "Fintech", value: 42, percentage: 10, percentageText: "10%" },
    { label: "Climate", value: 24, percentage: 6, percentageText: "6%" },
  ],
  recentUpdates: [],
  reviewCount: 12,
};

const DEV_ADMIN_UPDATES_STUB: VibeRaisingAdminUpdateSummary[] = [
  { id: "admin-update-1", startupName: "Neuronix AI", updateMonth: "June 2026", status: "published", lastUpdatedAt: "2026-06-06T10:24:00.000Z", founderName: "Sarah Chen", actionLabel: "View" },
  { id: "admin-update-2", startupName: "HealthBridge", updateMonth: "June 2026", status: "ready", lastUpdatedAt: "2026-06-06T09:11:00.000Z", founderName: "Michael Tran", actionLabel: "Review" },
  { id: "admin-update-3", startupName: "GreenLoop", updateMonth: "June 2026", status: "draft", lastUpdatedAt: "2026-06-05T16:32:00.000Z", founderName: "Aisha Patel", actionLabel: "Open Draft" },
  { id: "admin-update-4", startupName: "Spendwise", updateMonth: "May 2026", status: "published", lastUpdatedAt: "2026-06-04T14:18:00.000Z", founderName: "David Kim", actionLabel: "View" },
  { id: "admin-update-5", startupName: "Vesta Labs", updateMonth: "May 2026", status: "published", lastUpdatedAt: "2026-06-03T11:07:00.000Z", founderName: "Liam O'Connor", actionLabel: "View" },
];
DEV_ADMIN_OVERVIEW_STUB.recentUpdates = DEV_ADMIN_UPDATES_STUB;

function shouldUseAdminDevFallback(error: any): boolean {
  const status = error?.response?.status;
  return Boolean(
    shouldUseDevBackendStub() ||
      shouldUseDevBackendFallback(error) ||
      (import.meta.env.DEV && status === 404) ||
      (shouldUseDevAuthBypass() && (!error.response || status === 404)),
  );
}

export async function getVibeRaisingAdminOverview(
  env: Env,
  request: Request,
): Promise<VibeRaisingAdminOverview> {
  if (shouldUseDevBackendStub()) return DEV_ADMIN_OVERVIEW_STUB;

  try {
    const client = createApiClient(env, request);
    const response = await client.get(ADMIN_OVERVIEW_PATH);
    return normalizeVibeRaisingAdminOverview(response.data);
  } catch (error: any) {
    if (shouldUseAdminDevFallback(error)) {
      console.warn("Backend unavailable in local dev; using Vibe Raising admin overview stub.");
      return DEV_ADMIN_OVERVIEW_STUB;
    }
    throw error;
  }
}

export async function getVibeRaisingAdminUpdates(
  env: Env,
  request: Request,
  options: { status?: string | null; q?: string | null; page?: number | null } = {},
): Promise<VibeRaisingAdminUpdatesListResponse> {
  if (shouldUseDevBackendStub()) {
    const status = String(options.status || "").trim().toLowerCase();
    const q = String(options.q || "").trim().toLowerCase();
    const updates = DEV_ADMIN_UPDATES_STUB.filter((update) => {
      const matchesStatus = !status || update.status.toLowerCase() === status;
      const matchesQuery = !q || `${update.startupName} ${update.founderName || ""}`.toLowerCase().includes(q);
      return matchesStatus && matchesQuery;
    });
    return { updates, total: updates.length, page: 1, pageSize: updates.length, hasNext: false, hasPrevious: false };
  }
  ...
  const path = params.toString() ? `${ADMIN_UPDATES_PATH}?${params.toString()}` : ADMIN_UPDATES_PATH;

  try {
    const client = createApiClient(env, request);
    const response = await client.get(path);
    return normalizeVibeRaisingAdminUpdatesList(response.data);
  } catch (error: any) {
    if (shouldUseAdminDevFallback(error)) {
      console.warn("Backend unavailable in local dev; using Vibe Raising admin updates stub.");
      return { updates: DEV_ADMIN_UPDATES_STUB, total: DEV_ADMIN_UPDATES_STUB.length, page: 1, pageSize: DEV_ADMIN_UPDATES_STUB.length, hasNext: false, hasPrevious: false };
    }
    throw error;
  }
}

export async function getVibeRaisingAdminUpdateDetail(
  env: Env,
  request: Request,
  updateId: string,
): Promise<VibeRaisingAdminUpdateDetail> {
  if (shouldUseDevBackendStub()) {
    const summary = DEV_ADMIN_UPDATES_STUB.find((update) => update.id === updateId) ?? DEV_ADMIN_UPDATES_STUB[0];
    return {
      summary,
      update: DEV_MONTHLY_UPDATES_STUB[0] ?? null,
      company: null,
      founder: { name: summary.founderName, email: null },
    };
  }

  try {
    const client = createApiClient(env, request);
    const response = await client.get(`${ADMIN_UPDATES_PATH}${encodeURIComponent(updateId)}/`);
    return normalizeVibeRaisingAdminUpdateDetail(response.data);
  } catch (error: any) {
    if (shouldUseAdminDevFallback(error)) {
      console.warn("Backend unavailable in local dev; using Vibe Raising admin update detail stub.");
      const summary = DEV_ADMIN_UPDATES_STUB.find((update) => update.id === updateId) ?? DEV_ADMIN_UPDATES_STUB[0];
      return {
        summary,
        update: DEV_MONTHLY_UPDATES_STUB[0] ?? null,
...
function getVibeRaisingAdminEmails(env?: Env): Set<string> {
  const raw = String((env as Record<string, unknown> | undefined)?.VIBE_RAISING_ADMIN_EMAILS || "");
  return new Set(
    raw
      .split(/[;,\s]+/)
...
export async function requireVibeRaisingAdmin(
  env: Env,
  request: Request,
  ...
  if (!isVibeRaisingAdminUser(authUser, env)) {
...
const LOCAL_PUBLISHED_UPDATE_COOKIE = "vibe_local_published_update";
const LOCAL_DRAFT_UPDATE_COOKIE = "vibe_local_draft_update";

function readVibeRaisingLocalPublishedUpdate(request: Request): VibeRaisingMonthlyUpdate | null {
  if (!import.meta.env.DEV) return null;
  const cookieHeader = request.headers.get("Cookie") ?? "";
  const cookie = cookieHeader
    .split(";")
    .map((item) => item.trim())
    .find((item) => item.startsWith(`${LOCAL_PUBLISHED_UPDATE_COOKIE}=`));
  if (!cookie) return null;

  try {
    const raw = decodeURIComponent(cookie.slice(LOCAL_PUBLISHED_UPDATE_COOKIE.length + 1));
    return normalizeMonthlyUpdate(JSON.parse(raw));
  } catch {
    return null;
  }
}

function mergeLocalPublishedUpdate(
  bundle: VibeRaisingMonthlyUpdatesBundle,
  request: Request,
): VibeRaisingMonthlyUpdatesBundle {
  const localUpdate = readVibeRaisingLocalPublishedUpdate(request);
  if (!localUpdate) return bundle;
  const withoutDuplicate = bundle.updates.filter((update) => update.id !== localUpdate.id);
  return {
    ...bundle,
    updates: [localUpdate, ...withoutDuplicate],
  };
}
function readVibeRaisingLocalDraftUpdate(request: Request): VibeRaisingMonthlyUpdate | null {
  if (!import.meta.env.DEV) return null;
  const cookieHeader = request.headers.get("Cookie") ?? "";
  const cookie = cookieHeader
    .split(";")
    .map((item) => item.trim())
    .find((item) => item.startsWith(`${LOCAL_DRAFT_UPDATE_COOKIE}=`));
  if (!cookie) return null;

  try {
    const raw = decodeURIComponent(cookie.slice(LOCAL_DRAFT_UPDATE_COOKIE.length + 1));
    return normalizeMonthlyUpdate(JSON.parse(raw));
  } catch {
    return null;
  }
...
function mergeLocalDraftUpdate(
  drafts: VibeRaisingMonthlyUpdate[],
  request: Request,
): VibeRaisingMonthlyUpdate[] {
  const localDraft = readVibeRaisingLocalDraftUpdate(request);
  if (!localDraft) return drafts;
  const withoutDuplicate = drafts.filter((draft) => draft.id !== localDraft.id);
  return [localDraft, ...withoutDuplicate];
}

function compactLocalUpdate(update: VibeRaisingMonthlyUpdate): VibeRaisingMonthlyUpdate {
  const trimText = (value?: string | null) => String(value || "").slice(0, 900);
  return {
    id: update.id,
    isoMonth: update.isoMonth,
    month: update.month,
    ...
    summary: trimText(update.summary),
    sourceUrl: update.sourceUrl,
    metrics: update.metrics,
    displayConfig: update.displayConfig,
    highlights: trimText(update.highlights),
    challenges: trimText(update.challenges),
    asks: trimText(update.asks),
    learnings: trimText(update.learnings),
    next30Days: trimText(update.next30Days),
  };
}

function createVibeRaisingLocalUpdateCookie(
  name: string,
  update: VibeRaisingMonthlyUpdate,
): string | null {
  if (!import.meta.env.DEV) return null;
  const value = encodeURIComponent(JSON.stringify(compactLocalUpdate(update)));
  if (value.length > 3500) return null;
  return `${name}=${value}; Path=/founder-tools; Max-Age=3600; SameSite=Lax`;
}

export function createVibeRaisingLocalPublishedUpdateCookie(update: VibeRaisingMonthlyUpdate): string | null {
  return createVibeRaisingLocalUpdateCookie(LOCAL_PUBLISHED_UPDATE_COOKIE, update);
}

export function createVibeRaisingLocalDraftUpdateCookie(update: VibeRaisingMonthlyUpdate): string | null {
  return createVibeRaisingLocalUpdateCookie(LOCAL_DRAFT_UPDATE_COOKIE, update);
}
