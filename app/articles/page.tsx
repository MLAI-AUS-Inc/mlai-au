...
import { CATEGORY_SEGMENTS, getArticlesSortedNewestFirst, getFeaturedArticles, resolveArticleRouteSlug } from './registry'
...
function articleHref(slug: string) {
    return `/${routeBase}/${resolveArticleRouteSlug(slug)}`
}
                ...
                {featuredPreview.map(article => (
                    <article key={`featured-${article.slug}`} data-cf-slot="article-card" className="rounded-2xl border border-slate-200 bg-white px-5 py-5 shadow-sm">
                        ...
                        <h2 className="mt-3 text-xl font-semibold text-slate-950">
                            <a data-cf-slot="article-link" href={articleHref(article.slug)} className="hover:text-slate-700">
                                {article.title}
                            </a>
                        </h2>
