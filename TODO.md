...
- [x] Fix GSC "Page with redirect": repoint 8 article CTAs off legacy redirect slugs (/ai-founder-community-pitching-ideas, /practical-ai-learning-beginners-builders) straight to /articles
- [x] Align author/org schema host in `app/articles/authors.ts` from www.mlai.au to canonical mlai.au (stop reinforcing the www homepage redirect)
- [x] Remove the two now-dead legacy slugs from the article-internal-link validator whitelist so redirect-hop links regress-fail
- [x] Keep the /ai-founder-community-pitching-ideas and /practical-ai-learning-beginners-builders cleanup redirects in place for external/historical links
- [x] Verify: article internal-link check passes (125 files), `react-router typegen` + `tsc -b` clean (0 errors)
- [x] Fix GSC "Blocked by robots.txt": add `Allow: /platform/login` so Google can crawl the login page and honor its existing noindex meta (the robots block was silently defeating the noindex)
- [x] Keep /platform/dashboard, /platform/logout, /hospital/, /esafety/, /verify-email blocked; /founder-tools/ resolves because it 302s into the now-allowed login
- [x] Fix GSC "Not found (404)": add `/$` -> `/` cleanup (route + map, mirroring `/&`) and redirect the literal `/articles/*` glob -> `/articles` in the articles splat loader
- [x] Verify locally (dev server): `/$` and `/articles/*` both 301 to `/` and `/articles`; genuinely missing articles still return a real 404 (no soft-404); `tsc -b` clean, link validator passes (125 files)
- [x] Fix GSC "Duplicate without user-selected canonical" (/vibe-raising/): redirect /vibe-raising/ and /vibe-raising-landing -> /vibe-raising at the worker (LEGACY_REDIRECTS), and remove the colliding trailing-slash + legacy-landing routes from routes.ts (the trailing-slash route was shadowing /vibe-raising and 404ing it)
- [x] Verify locally (dev server): /vibe-raising 200, /vibe-raising/ and /vibe-raising-landing 301 -> /vibe-raising (1 hop, no loop); tsc -b clean, link validator passes
