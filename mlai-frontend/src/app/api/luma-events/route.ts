    ...
    try {
        const response = await fetch('https://api.lu.ma/search/get-results?query=', {
            headers: {
                ...
                'X-Luma-Web-Url': 'https://lu.ma/calendar/manage/cal-KPakbH2wTxQuyCj?period=past'
            }
        });
        ...
        const transformedEvents = data.events?.map((item: any) => ({
            ...
            location: item.event.geo_address_info?.full_address || item.event.geo_address_info?.city_state || '',
