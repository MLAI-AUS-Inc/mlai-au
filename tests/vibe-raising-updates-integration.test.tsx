...
describe("updates remain inside the existing Vibe Raising application", () => {
  ...
  it("keeps AI drafting, the default cover and dot-point editing inside the existing app", () => {
    const html = renderPage("/founder-tools/updates/create?edit=1");
    expect(html).toContain('title="Active company: MLAI"');
    expect(html).toContain('aria-label="Update progress"');
    for (const label of ["Draft", "Refine", "Review", "Your connected sources", "Default cover", "Generate image", "Add point"]) expect(html).toContain(label);
    expect(html.indexOf("Your connected sources")).toBeLessThan(html.indexOf("Your draft"));
