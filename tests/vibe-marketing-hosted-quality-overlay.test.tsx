...
function sixthShapedRun() {
  return normalizeMarketingRun({
    ...
    hostedQualityIssues: [
      { id: "hosted:claim-150", claimId: "claim-150", componentId: "resource-cta", sectionId: null,
...
describe("hosted article quality issue markers", () => {
  test("shows sixth-shaped image and resource findings without inventing a disclosure anchor", () => {
    const issues = currentHostedQualityIssuesForRun(sixthShapedRun());
    expect(issues.map((issue) => issue.componentId)).toEqual(["resource-cta", "image:test-assistant", null]);
    const markup = renderToStaticMarkup(createElement(ArticleHostedQualityIssueOverlay, {
      issues,
      measurements: {
        "resource-cta": { id: "resource-cta", type: "resource-cta", label: "Worksheet download",
          rect: { left: 10, top: 40, right: 410, bottom: 240, width: 400, height: 200 } },
    ...
    expect(markup).toContain('aria-label="Review quality finding for Worksheet download"');
  ...
  test("keeps unanchored disclosure finding visible in the review banner", () => {
